...
In the first pass, the script traverses every row of the dataset and computes three core metrics for each ticker: absolute yearly price change, percentage price change, and cumulative trading volume. Yearly price change is derived by differencing the ticker's first recorded opening price against its last recorded closing price — anchoring the calculation to actual market open and close rather than arbitrary intermediate values. Percentage change is then computed from this difference, normalizing returns across tickers regardless of their nominal price scale. Cumulative volume is accumulated row by row across all trading sessions for the year. These three metrics are written directly to a structured summary table co-located with the source data, keeping inputs and outputs on the same worksheet for straightforward auditing and cross-reference.

The second pass operates on the completed summary table rather than the raw records, scanning the per-ticker metrics to identify three market-wide highlights: the ticker with the greatest percentage gain, the ticker with the greatest percentage loss, and the ticker with the highest cumulative trading volume. These findings are written to a dedicated highlights table, delivering an at-a-glance snapshot of the year's standout extremes without requiring any manual filtering or sorting.
